...
export default function ExplSec() {
  return (
    <section className="mt-12 mb-40 lg:mt-24 lg:mb-60 container">
      <h2 className="w-fit text-sm lg:text-base font-bold lg:mb-5 mb-6 rounded-lg bg-gold-grad text-black px-6 py-3">
        About Credits
      </h2>
      <h3 className="text-[#E0D254] mb-12 text-base lg:text-lg font-bold lg:mb-5">
      ...
      <div className=" mt-12 lg:mt-9 space-y-9 lg:space-y-12">
        {/* TODO change the pre tag in the item component */}
        <ExplItem title="Domain Section" text={text} />
        <ExplItem title="Leak Section" text={text} />
        <ExplItem title="Logs Section" text={text} />
        <ExplItem title="Practical Examples" text={text} />
      </div>

      <h2
        className="sm:w-fit text-justify sm:rounded-full bg-purple-300/20 text-xs py-3 px-9 lg:text-sm  z-30
