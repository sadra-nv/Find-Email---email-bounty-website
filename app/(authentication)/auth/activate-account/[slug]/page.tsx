import { verifyRegisteringUser } from "@/lib/services/auth/verifyRegisteringUser";
// import { Button } from "@headlessui/react";
import Link from "next/link";

export default async function page({ params }: { params: { slug: string } }) {
  const result = await verifyRegisteringUser(params.slug);
  ...
  return (
    ...
    >
      ...
      <div className="w-full relative z-20 h-full flex flex-col justify-start items-center">
        <h1 className="font-bold text-sm sm:text-3xl leading-7 sm:leading-10 text-center w-11/12 lg:w-5/12 mb-6 sm:mb-8 relative z-30">
          {result && result.message}
          {!result &&
            "Failed to get any response from the server, Please retry"}
        </h1>
        ...
        <Link
          className="block py-3 px-10 bg-blue-grad-btn rounded-lg mt-8 sm:mt-14 text-xs sm:text-sm font-semibold"
          href="/auth"
        >
          Login
        </Link>
